...
- `cli.py` wires the commands: `setup`, `roster`, `power`, `values`, `lineup`, `trade`, `movers`, `waivers`, `cleanup`, `draft`. The `@_guard` decorator turns the two expected real-world failures (unreachable API, corrupt config) into clean one-line errors while preserving each command's Typer signature.
