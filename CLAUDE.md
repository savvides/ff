...
- Dynasty Dealer (`dynastydealer.com`): optional secondary-market values, merged onto the same `Asset` as `secondary_value` (with `dealer_value` and `ktc_value` aliases).
...
- `values/client.py` fetches FantasyCalc and builds a `ValueBook` with three lookups: by sleeper id (exact, for rosters), by name (fuzzy, for typed trade input), and by pick label. `values/dealer.py` fetches Dynasty Dealer values; `values/normalize.py` holds `normalize_name`/`normalize_pick` so client and dealer do not import each other.
